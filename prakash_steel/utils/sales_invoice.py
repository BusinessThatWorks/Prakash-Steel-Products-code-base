...
# 					_("Row {0}: Item {1}").format(idx, item.item_code or item.item_name or _("N/A"))
# 				)

# 		if invalid_items:
# 			error_message = _("Only items linked to Sales Order are allowed in this Sales Invoice.")
...
# 		# Success message
# 		frappe.msgprint(
# 			_("Stock Entries created successfully: {0} (Material Receipt) and {1} (Material Issue)").format(
# 				material_receipt.name, material_issue.name
# 			),
# 			indicator="green",
# 		)
...
# 		frappe.log_error(
# 			title=_("Stock Entry Creation Failed for Sales Invoice {0}").format(doc.name),
# 			message=frappe.get_traceback(),
# 		)
# 		frappe.throw(
# 			_("Failed to create Stock Entries for Sales Invoice {0}. Error: {1}").format(doc.name, str(e))
# 		)
...
# 	stock_entry.remarks = _("Material Receipt created automatically from Sales Invoice {0}").format(
# 		sales_invoice.name
# 	)
...
# 		frappe.throw(
# 			_("No items with custom_tolerance_qty found in Sales Invoice {0}").format(sales_invoice.name)
# 		)
...
# 	# Set remarks
# 	stock_entry.set_posting_time = 1
# 	stock_entry.remarks = _("Material Issue created automatically from Sales Invoice {0}").format(
# 		sales_invoice.name
# 	)
