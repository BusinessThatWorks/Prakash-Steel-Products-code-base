...
def get_data(filters):
	...
	query = (
		...
		.left_join(item)
		.on(item.name == po_item.item_code)
		.select(
			...
			po.supplier,
			po_item.item_code,
			item.custom_category_name.as_("category_name"),
			po_item.rate,
			po_item.qty,
...
def prepare_data(data, filters):
	...
	for row in data:
		...
		# calculate CF Lead Time: difference between Required By and Date
		# Example: Date = 26-10-2025, Required By = 28-10-2025 → cf_lead_time = 2
		if row.get("date") and row.get("required_date"):
			row["cf_lead_time"] = date_diff(row["required_date"], row["date"])
		else:
			row["cf_lead_time"] = None
