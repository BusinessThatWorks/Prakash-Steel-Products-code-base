...
def generate_mrp_order_recommendations():
    ...
    frappe.cache().set_value(
        f"mrp_job_id_{frappe.session.user}", job_id, expires_in_sec=3600
    )

    # Log job creation
    print(
        f"[MRP] Job queued with ID: {job_id}, Name: MRP Generation - {frappe.session.user}"
    )
...
def _generate_mrp_order_recommendations_worker():
    ...
    for item_code in all_item_codes:
        ...
        net_order_rec = calculate_net_order_recommendation(
            base_order_rec, moq, batch_size
        )
    ...
    for item_code in all_item_codes:
        ...
        if item_code in detailed_info:
            detailed_info[item_code]["total_parent_demand"] = flt(
                parent_demand_map.get(item_code, 0)
            )
    ...
    for item_code in all_item_codes:
        ...
        build_calculation_breakdown(
            info, parent_demand_map_net
        )  # Use updated parent demands

        # Verify breakdown was created (for debugging)
        if (
            not info.get("calculation_breakdown")
            or not info["calculation_breakdown"].strip()
        ):
            frappe.log_error(
    ...
    if job_id:
        ...
        frappe.cache().set_value(
            cache_key, result, expires_in_sec=3600
        )  # Store for 1 hour

    # Log job completion
    items_with_rec = len(
        [qty for qty in net_order_recommendations_final.values() if flt(qty) > 0]
    )
    completion_msg = f"[MRP Job {job_id or 'Unknown'}] Completed! Items with net order rec > 0: {items_with_rec}"
...
def get_mrp_job_status(job_id):
    ...
    try:
        ...
        result = frappe.cache().get_value(cache_key_mrp) or frappe.cache().get_value(
            cache_key_mr
        )
            ...
            active_jobs.append(
                {
                    ...
                    if job.is_finished
                        ...
                        "failed"
                        if job.is_failed
                        else ("started" if job.is_started else "queued")
                    ),
...
def create_material_requests_automatically(net_order_recommendations=None):
    ...
    if net_order_recommendations is None:
        ...
        job_id = job.id if hasattr(job, "id") else str(job)
        frappe.cache().set_value(
            f"mr_creation_job_id_{frappe.session.user}", job_id, expires_in_sec=1800
        )
        ...
        frappe.cache().set_value(
            cache_key, net_order_recommendations, expires_in_sec=1800
        )
...
def _create_material_requests_worker(net_order_recommendations=None):
    ...
    start_msg = (
        f"[MR Creation Job {job_id or 'Unknown'}] Starting Material Request creation..."
    )
    print(start_msg)
    ...
    items_to_process = [
        (item_code, qty)
        for item_code, qty in net_order_recommendations.items()
        if flt(qty) > 0
    ]
            ...
            if batch_size > 0:
                ...
                for i in range(num_requests):
                    ...
                    if result.get("error"):
                        error_count += 1
                        errors.append(
                            f"{item_code} (MR #{i + 1} of {num_requests}): {result.get('error')}"
                        )
                    else:
                        success_count += 1
                ...
                if remainder > 0:
                    ...
                    if result.get("error"):
                        error_count += 1
                        errors.append(
                            f"{item_code} (Remainder {remainder}): {result.get('error')}"
                        )
                    else:
                        success_count += 1
                        material_requests.append(result.get("material_request"))
    ...
    result = {
        ...
        "errors": errors[:10]
        if len(errors) > 10
        else errors,  # Limit errors to first 10
    ...
    if job_id:
        ...
        frappe.cache().set_value(
            cache_key, result, expires_in_sec=1800
...
def traverse_bom_for_mrp(
    ...
    try:
        ...
        bom_quantity = flt(
            bom_doc.quantity
        )  # Quantity of parent item produced by this BOM
        if bom_quantity <= 0:
        ...
        for bom_item in bom_doc.items:
            ...
            if child_item_code not in detailed_info:
                ...
                detailed_info[child_item_code] = {
                    ...
                    "qualified_demand": flt(
                        qualified_demand_map.get(child_item_code, 0)
                    ),
            ...
            if is_child_buffer:
                ...
                if child_sku_type in ["BOTA", "PTA"]:
                    ...
                    base_child_order_rec = (
                        child_tog + child_qualified_demand - child_stock - child_wip
                    )
                ...
                if child_sku_type in ["PTO", "BOTO"]:
                    # For PTO/BOTO: Requirement - Stock - WIP - Open PO - MRQ
                    base_child_order_rec = (
                        child_requirement - child_stock - child_wip - child_open_po
                    )
                else:
                    # For other non-buffer items: Requirement - Stock - WIP - MRQ
...
def build_calculation_breakdown(info, parent_demand_map):
    ...
    if is_buffer:
        ...
        if sku_type in ["BOTA", "PTA"]:
            ...
            lines.append(
                "  Base Calculation: TOG + Qualified Demand - Stock - WIP - Open PO"
            )
            lines.append(
                f"                    = {tog} + {qualified_demand} - {stock} - {wip} - {open_po}"
            )
            lines.append(f"                    = {base_calc}")
            lines.append(
                f"  After MRQ: Base - MRQ = {base_calc} - {mrq} = {final_order_rec}"
            )
        else:
            base_calc = tog + qualified_demand - stock - wip
            lines.append("  Base Calculation: TOG + Qualified Demand - Stock - WIP")
            lines.append(
                f"                    = {tog} + {qualified_demand} - {stock} - {wip}"
            )
            lines.append(f"                    = {base_calc}")
            lines.append(
                f"  After MRQ: Base - MRQ = {base_calc} - {mrq} = {final_order_rec}"
        ...
        if final_order_rec <= 0:
            ...
            if moq > 0:
                lines.append(
                    f"  MOQ: {moq} (Not applied - base order rec is {final_order_rec} <= 0)"
                )
            elif batch_size > 0:
                lines.append(
            ...
            lines.append(
                f"  Net Order Recommendation: 0 (Base order rec is {final_order_rec} <= 0)"
            )
        elif moq > 0:
        ...
        if sku_type in ["PTO", "BOTO"]:
            ...
            lines.append(
                f"                    = {requirement} - {stock} - {wip} - {open_po}"
            )
            lines.append(f"                    = {base_calc}")
            lines.append(
                f"  After MRQ: Base - MRQ = {base_calc} - {mrq} = {final_order_rec}"
...
def generate_detailed_log(detailed_info, net_order_recommendations):
    ...
    lines.append("\n" + "=" * 100)
    lines.append(
        "MRP ORDER RECOMMENDATIONS - DETAILED BREAKDOWN (NET ORDER RECOMMENDATIONS)"
    )
    lines.append("=" * 100)
    ...
    if total_items != items_in_detailed_info:
        lines.append(
            f"WARNING: {total_items - items_in_detailed_info} items missing from detailed_info!"
        )
    ...
    for item_code, net_rec in items_with_net_rec:
        ...
        lines.append(
            f"  {item_code} ({buffer_flag}): Net Order Rec = {net_rec} (Base: {final_rec})"
        )
