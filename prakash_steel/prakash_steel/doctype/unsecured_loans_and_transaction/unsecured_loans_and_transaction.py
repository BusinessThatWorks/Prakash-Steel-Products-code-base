...
class UnsecuredLoansandTransaction(Document):
    ...
    def validate(self):
        """Document save hone se pehle sab kuch set karo"""
        self.set_from_to_dates()
        self.populate_interest_details()

    def set_from_to_dates(self):
        """Financial Year se From/To Date set karo"""
    ...
    def populate_interest_details(self):
        ...
        if month_start_date > today_date:
            return

       
        daily_rate_decimal = annual_rate / 100.0 / 12.0 / 30.0
        ...
        while current <= today_date:
            ...
            interest_amt = flt(closing_bal) * daily_rate_decimal

            self.append("interest_details", {
                "date": current,
                "closing_balance": closing_bal,
                "day_interest": round(daily_rate_decimal * 100, 6),
...
def fetch_daily_interest_for_all_active_docs():
    """Raat 11:55 par run hoga - daily entry add karega"""
    today_date = getdate(today())
